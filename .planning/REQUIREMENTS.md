...
| DS-AUTH-04 | URL-shape allow-list of one — parent only fetches URLs matching the Phase 02 proxy URL template `/<restRoot>/maps_plus/tile/proxy?url=…&z=…&x=…&y=…[&s=…][&r=…]`; all non-conforming URLs rejected before any fetch is dispatched | Must | Encoded as a single validated regex shared between iframe and parent (or pinned via Jest across both sides). **Parent:** non-conforming URLs are **silently dropped** (no `postMessage` response, no `fetch`) — same signal discipline as D-05 rate-limit silent drop, so a hostile iframe cannot distinguish shape-rejection from other drops. **Iframe:** the pending tile’s **8000ms** per-request timeout (D-06) resolves as `{ ok: false, code: 'timeout' }` / blank tile when no response arrives. No outbound `fetch` on the parent for rejected shapes. |
