...
<audit_findings>
## Audit of qwen3.6 Reference Plan (`01-PLAN.qwen-reference.md`)

The previous plan, generated by a local LLM (qwen3.6), was reviewed and
preserved as `01-PLAN.qwen-reference.md` for comparison. Its threat model
(T1–T6), task decomposition skeleton, and dependency graph are sound and
worth reusing. The replan MUST address the following concrete defects:

### Critical defects (must fix)
- **A-01:** qwen plan T1.2.2 shipped `"allowed_domains": []` with comment
  "empty list means allow all". This is a default-allow SSRF. CONTEXT.md
  D-02 overrides: empty list = deny-all. Seed with REQ-DS-05 hosts.
- **A-02:** qwen plan names SSRF mitigations (allowlist, private-IP block,
  https-only) in T1 but implements **no** task for any of them. The new plan
  MUST have a dedicated task for URL validation executed BEFORE `_fetch_tile()`:
  (a) scheme check (https only, D-09), (b) host allowlist match with
  leftmost-wildcard (D-01/D-03), (c) DNS resolution + private/reserved IP
  block via `ipaddress.ip_address()` against the resolved A/AAAA records.
- **A-03:** qwen plan T1.3.2 used `open(path, 'xb')` (exclusive create). This
  fails on every cache refresh after the first write. Use `'wb'`. Concurrency
  concerns are handled by atomic write-then-rename if needed.

### Major defects (likely wrong on Splunk 9.x)
- **A-04:** qwen plan T1.2.1 uses `[route:...]` + `source=` stanza. Splunk's
  actual modular REST syntax is `[script:<name>]` with `match=`, `script=`,
  `scripttype=persist`, `handler=`, `python.version=python3`,
  `requireAuthentication=true`. **Replan task must research via Context7 or
  Splunk docs before prescribing syntax.**
- **A-05:** qwen plan chose `BaseRestHandler` as the handler base class. For
  binary responses (tile bytes) the idiomatic class is
  `splunk.persistconn.application.PersistentServerConnectionApplication`,
  which returns `{"payload": bytes, "status": int, "headers": {...}}`.
  **Replan must verify which base class is required for binary content and
  pick that one.**
- **A-06:** qwen plan T1.2.4 depends on `pytest`. PROJECT.md constraint is
  stdlib-only. Use `unittest` (stdlib) — runner script invokes
  `python -m unittest discover tests/ -v`.
- **A-07:** qwen plan T1.1.2 auto-swaps `{x}` and `{y}` when the URL contains
  `/ArcGIS/` or `gibs.earthdata.nasa.gov`. This is a surprising
  server-side mutation. **Remove.** The client sends the template it wants
  resolved verbatim; server performs only literal placeholder substitution.
- **A-08:** qwen plan T1.1.4 forced `Content-Type: image/png` when upstream
  value wasn't png/jpeg. This breaks WebP (CartoDB, Stadia). Per D-15:
  pass through upstream's Content-Type; default to
  `application/octet-stream` only if upstream omits it.
- **A-09:** qwen plan settings.json documents `enabled` toggle but no task
  wires the 503-when-disabled check. Replan MUST include a task that checks
  the enabled flag at the top of the GET handler and returns 503 with
  `{"error":"proxy disabled"}` when false.

### Minor defects (cleanups)
- **A-10:** qwen plan T1.1.1 verification imports `tile_proxy` which T1.1.2
  creates — verification is in the wrong task. Move.
- **A-11:** `hashlib.sha256(...).hexdigest()[:64]` is a no-op (SHA-256 is
  already 64 hex chars). Drop the slice.
- **A-12:** qwen plan T1.2.5 updated only
  `appserver/.../scripts/deploy.sh` (dev Splunk Docker deploy). The release
  packaging script `build_release.sh` at repo root ALSO packages the app
  and must be updated to include `bin/`. Both must change.
- **A-13:** All Splunk 9.x REST stanzas require `python.version = python3`.
  Easy to miss; call it out explicitly in the plan's restmap task.

### Structural inheritance (keep from qwen plan)
- Threat model T1–T6 (with T1 mitigation reworded per D-02)
- Three-plan decomposition: 1.1 handler, 1.2 config, 1.3 disk cache
- Task granularity (~8–9 tasks per plan is about right)
- Test-infrastructure-first ordering
...
*Mode: `--auto` — 7 gray areas auto-resolved to recommended defaults; audit
of qwen3.6 reference plan appended 2026-04-16 after review.*
*Replan instruction: read this entire file AND `01-PLAN.qwen-reference.md`
before writing the new plan. Inherit the structure, fix the audit findings.*
